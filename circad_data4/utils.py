...
# Get positive and negative indexes from adjacency matrix
...
# Generate 5-fold splits for training and testing
def gen_folds(adj):
...
# Function for Smith-Waterman alignment
...
# Logger class to track training metrics and save to file
class Logger:
    def __init__(self, total_fold, log_file="training_log.csv"):
        # Specify log file
        self.log_file = log_file

        # Create the CSV file and write header if not exists
        if not os.path.exists(self.log_file):
            with open(self.log_file, 'w') as f:
                f.write("fold, epoch, f1_score, f2_score, rank_idx, auc, aupr, threshold, recall, precision, acc, specificity, mcc, train_loss, test_loss\n")
    ...
    # Update metrics at each epoch and save to CSV
    def update(self, fold, epoch, adj, pred, test_idx, train_loss, test_loss):
        ...
        with open(self.log_file, 'a') as f:
            f.write(f"{fold}, {epoch}, {f1_score}, {f2_score}, {rank_idx}, {auc}, "
                    f"{aupr}, {threshold}, {recall}, {precision}, {acc}, {specificity}, "
                    f"{mcc}, {int(train_loss)}, {int(test_loss)}\n")
        
        # 返回AUC用于early stopping和scheduler
        return auc
