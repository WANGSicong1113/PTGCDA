...
class GAT(nn.Module):
    """
    GAT: Graph Attention Network for node feature aggregation.
    
    Uses multi-head attention mechanism to aggregate neighbor features.
    """
    def __init__(self, p_feat_dim, d_feat_dim, n_hidden, dropout, alpha=0.2, nheads=2):
        ...
        assert n_hidden % nheads == 0
        nhid_per_head = int(n_hidden / nheads)
        self.layer1 = [
            GraphAttentionLayer(n_hidden, nhid_per_head, dropout=dropout, alpha=alpha)
        ...
        self.out_att = GraphAttentionLayer(
            nhid_per_head * nheads, n_hidden, dropout=dropout, alpha=alpha
        )
...
class PUTransGCN(nn.Module):
    ...
    2. GAT: Aggregates graph structure information using attention
