...
class GraphSAGE(nn.Module):
    """
    GraphSAGE: Graph neural network for circRNA-disease node feature aggregation.
    """
    def __init__(self, p_feat_dim, d_feat_dim, n_hidden, dropout, num_layers=1):
        ...
        # Simplified: single linear layer (concat self + aggregated), Output: n_hidden
        self.graph_linear = nn.Linear(2 * n_hidden, n_hidden)
    ...
    def forward(self, p_feat, d_feat, adj):
        ...
        x_new = self.graph_linear(x_concat)
        x = x + x_new 
...
class PTGCDA(nn.Module):
    """
    PTGCDA: Main model for circRNA-disease association prediction.
    ...
    """
    def __init__(self, deep_lnc_loc, graph_sage, p_encoder, d_encoder, predictor, **kwargs):
        super(PTGCDA, self).__init__(**kwargs)
    ...
    def forward(self, p_kmers_id, d_feat, adj_mat):
        ...
        p_feat_gcn, d_feat_gcn = self.graph_sage(p_feat_dll, d_feat, adj_mat)
